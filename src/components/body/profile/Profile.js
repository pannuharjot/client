...
import { validPassword, isMatch } from '../../utils/validations/Validation'
...
const Profile = () => {
    ...
    const valid = name.length <= 3 || name === 'undefined' || name  === null;
   ...
   const updateInfor = () => {
    ...
    if(valid) return setData({...data, err: 'Name field cannot be empty!', success: '' })
   ...
   const updatePassword = () => {
       if(!validPassword(password)) {
           return setData({...data, err: 'Invalid Password', success: ''})
       }
   ...
   const handleDelete = async (id) => {
    ...
    try {
        if(user._id == id) return setData({...data, err: 'Unable to delete own account!', success: ''})
